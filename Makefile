...
run:
	docker run -d -p 8081:80 -v /var/run/docker.sock:/var/run/docker.sock -v `pwd`/webroot:/var/www --env VERSION_HOSTS=stage-businesslogic.internal.blackwoodseven.com:81\;localhost:8080 --name $(NAME) -t $(REPO)
